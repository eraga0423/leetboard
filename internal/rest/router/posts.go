...
func (r *Router) post() {
	...
	r.router.Handle("GET /post/{id}", r.midd.Logger(r.midd.Authentificator(http.HandlerFunc(r.handler.GetPostID))))
	r.router.Handle("POST /create", r.midd.Authentificator(http.HandlerFunc(r.handler.PostMethodCreatePost)))
	r.router.Handle("POST /post/{id}", r.midd.Logger(r.midd.Authentificator(http.HandlerFunc(r.handler.NewComment)))) //////////
}
